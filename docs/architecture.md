...
**2026-09-06（M15–M16）**：三台各自是 **MCP server**（`device/mcp_server.py`：tools＝儀器程序一對一、resources `device://*`、共享 bearer token、兩段式 confirm、與 worker 同行程；`emforge[mcp]` optional extra，核心 import 期零 mcp）——這是 §10「AI 加值層」進儀器的**第三條**控制路徑（MCP／CLI `device-*`／Python API），迴圈本身仍然沒有 LLM。§10 假設 AI 只讀狀態、寫提案：現在 AI 還能對單台下單跑一筆（不入 db）、按急停（不能解）、停續 worker——全部有人在迴路的閘（confirm token、CLI-only 解除、allowed_profiles）。
