...
def test_dispatch_writes_inflight_before_job(rt, monkeypatch):
    ...
    assert paths.inflight_file(rt.root, "fake_f1", "fake_f1-blind-t00001").exists()
...
def test_dispatch_drops_db_inflight_and_intra_batch_dups_counts_dup_dropped(rt):
    ...
    ev = [e for e in fs.read_jsonl(paths.events_jsonl(rt.root, "fake_f1")) if e["event"] == "proposals_validated"]
...
def test_dispatch_all_dups_returns_none_and_writes_nothing(rt):
    ...
    assert not paths.inflight_file(rt.root, "fake_f1", "fake_f1-blind-t00002").exists()
...
def test_dispatch_repeat_kind_skips_dedup_and_uses_given_store(rt):
    ...
    assert fs.read_json(paths.inflight_file(rt.root, "fake_f1", s1))["kind"] == "repeat"
