...
def gate(job: Job, depot) -> GateVerdict:
    ...
    return check(job.sim_profile, job.profile_hash, depot)


def check(sim_profile: str, profile_hash: str, depot) -> GateVerdict:
    """守門本體（不需要 Job 物件；儀器層的前置檢查也用它）。"""
    try:
        profile = profiles.get_profile(sim_profile)
    ...
    if profile_hash != profile.profile_hash:
        return GateVerdict(False, f"profile_hash_mismatch: job={profile_hash} registry={profile.profile_hash}"
