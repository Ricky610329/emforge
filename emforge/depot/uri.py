...
`file://<root>`（`file://C:/x`、`file:///C:/x`、`file:///home/x`、`file:////server/share`）／`memory://<name>`（行程級單例）／
...
from .memory import MemoryDepot, registered
...
def open_depot(spec) -> Depot:
    ...
    if scheme == "memory":
        ...
        return registered(rest) or MemoryDepot(name=rest)
